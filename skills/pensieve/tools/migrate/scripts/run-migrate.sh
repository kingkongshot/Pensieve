...
# 1) Migrate legacy user-data paths into project root
...
"$PYTHON_BIN" - "$ROOT" "$PROJECT_ROOT" "$PLUGIN_ROOT" "${HOME:-}" "$BACKUP_DIR" "$MIGRATION_SUMMARY" "$DRY_RUN" "$TIMESTAMP" "$SCHEMA_FILE" <<'PY'
...
for item in schema.get("critical_files", []):
    ...
    if not isinstance(target, str) or not isinstance(template, str):
        continue
    critical_pairs.append((root / target, plugin_skill_root / template))
