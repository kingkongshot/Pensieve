...
### Claude Code 插件 + Skill 模式（推荐）

分两步：先装插件获取 hooks 自动化能力，再装 skill 获取内容。**无需 clone 仓库**：

```bash
# 1. 装插件（hooks：SessionStart / PreToolUse / PostToolUse）
...
# 2. 装 skill（内容：.src/、SKILL.md、种子数据）
npx skills add https://github.com/kingkongshot/Pensieve/tree/experimental/skills/pensieve --copy -a claude-code -y
```

> **提示**：插件命令执行成功后**不会有任何输出**，这是正常行为。可以通过以下方式确认：
>
> ```bash
> claude plugin list                # 确认插件已安装
> ls .claude/skills/pensieve/.src   # 确认 skill 内容已就位
> ```
...
npx skills add /path/to/Pensieve/skills/pensieve -a claude-code --copy
```
...
```bash
npx skills add https://github.com/kingkongshot/Pensieve/tree/experimental/skills/pensieve --copy
```

> **注意**：分支通过 URL 中的 `tree/<branch>` 指定，`--branch` 不是 skills CLI 的有效参数。
