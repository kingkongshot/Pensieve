...
4. 统计 diff 总行数（`git diff --stat` 或用户指定范围），分级：
   - **S**（<50 行）：轻量审查
   - **M**（50-200 行）：标准审查
   - **L**（>200 行）：深度审查

**完成标准**：得到可执行待审范围 + 可沉淀候选清单（均有证据）+ diff 规模分级
...
**执行步骤**：
1. 根据 Task 1 的 diff 规模分级决定审查广度：
   - **S 级**：仅审查正确性、安全性
   - **M 级**：S + 性能（时间/空间复杂度、N+1 查询）、可测试性、接口设计（浅模块/信息泄漏）
   - **L 级**：M + 对抗性审查（主动构造破坏性输入/状态）+ 架构影响评估（跨模块影响、新依赖、数据流变更）
2. 对每个文件按当前审查广度执行审查清单（理论与依据见 Knowledge，不在此文件复制）
3. 仅记录”可能真实”的候选问题，附置信度（0-100）
4. 对每条候选问题标注精确代码位置与证据
5. 记录用户可见行为变化风险（若有）
